...
contract ResolverMultisigTest is Test {
    ...
    function test_targets_startEmptyAndAreAdminManaged() public {
        ...
        vm.prank(alice);
        vm.expectRevert(abi.encodeWithSelector(ResolverMultisig.TargetNotAllowed.selector, address(resolver)));
    ...
    function test_addTarget_rejectsDuplicatesAndZero() public {
        vm.prank(operator);
        vm.expectRevert(abi.encodeWithSelector(ResolverMultisig.AlreadyTarget.selector, address(resolver)));
        sig.addTarget(address(resolver));
    ...
    function test_propose_rejectsAnUnadoptedTarget() public {
        vm.prank(alice);
        vm.expectRevert(abi.encodeWithSelector(ResolverMultisig.TargetNotAllowed.selector, address(market)));
    ...
    function test_thresholdOfOneSettlesOnPropose() public {
        ...
        vm.prank(alice);
        solo.propose(
            address(soloResolver), abi.encodeCall(TrustedResolver.resolveMarket, (address(soloMarket), 1, 0))
        );
    ...
    function test_failingTargetCallRevertsTheWholeTransaction() public {
        ...
        bytes memory data = abi.encodeCall(TrustedResolver.resolveMarket, (address(foreign), 1, 0));
